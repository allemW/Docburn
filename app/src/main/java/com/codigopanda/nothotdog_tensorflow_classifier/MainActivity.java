...
import com.codigopanda.nothotdog_tensorflow_classifier.util.ImageUtils;
...
public class MainActivity extends AppCompatActivity implements ImageReader.OnImageAvailableListener, CameraFragment.ConnectionCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        publicar = (Button) findViewById(R.id.btntratamiento1);

        publicar.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent i = new Intent(MainActivity.this,Tiposquemaduras.class);
                startActivity(i);
            }
        });
    }
    ...
    public void onImageAvailable(final ImageReader reader) {
        ...
        respuesta=(TextView) findViewById(R.id.textrespuesta);
                ...
                new Runnable() {
                    ...
                    public void run() {
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                if (results.size() > 0) {
                                    String cad = results.get(0).toString();
                                    respuesta.setText("");

                                  /*  for (int i=0;i<results.size();i++){
                                        respuesta.setText(respuesta.getText().toString()+ results.get(i).title);
                                    }*/
                                    Recognition aux= results.get(0);



                                    respuesta.setText(aux.title);
                                    respuesta.setBackgroundColor(Color.GREEN);
                                    if(aux.title.compareTo("primer grado")==0&&(aux.confidence * 100.0f)>95.0f){
                                        respuesta.setText("quemadura primer grado!");
                                        respuesta.setBackgroundColor(Color.GREEN);
                                    }else{

                                        if(aux.title.compareTo("tercer grado")==0&&(aux.confidence * 100.0f)>95.0f){
                                            respuesta.setText("quemadura tercer grado!");
                                            respuesta.setBackgroundColor(Color.GREEN);
                                        }else {
                                            respuesta.setText("No es quemadura");
                                            respuesta.setBackgroundColor(Color.RED);
                                        }
                                    }

                                    resultsView.setText(cad);
                                } else {
